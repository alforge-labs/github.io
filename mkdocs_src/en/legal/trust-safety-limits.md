...
During authentication, only information required by the Whop OAuth 2.0 PKCE flow is sent to the external auth service. When you run `alpha-forge system auth login`, the authorization code and PKCE parameters are sent, and the response returns OAuth tokens together with your Whop user ID and membership status.
...
- [EULA](eula.md)
- [Trial Limits](../guides/trial-limits.md)
