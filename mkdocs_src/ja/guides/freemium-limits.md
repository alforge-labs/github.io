...
AlphaForge には **Free / Monthly / Annual / Lifetime** の 4 プランがあります。Free 以外（Lifetime / Annual / Monthly）はまとめて**有料プラン**と呼びます。Free プランでは、評価エンジン（バックテスト・最適化）に渡せるデータ日付の上限が **2023-12-31** に制限されており、また最適化のトライアル数が **50 回** に制限されます。本ページでは、その挙動と確認方法を整理します。
...
!!! note "対象コマンド"
    ...
    取得時にも評価時にも **2023-12-31** をキャップとして共有し、最適化系コマンドは **50 trials** をキャップとして共有しています。Pine Script エクスポートは Free プランでは**完全にブロック**されます。

## プラン構成

| プラン | データ取得・評価の日付制限 | 最適化 trial 数 | 補足 |
|---|---|---|---|
| Free | 2023-12-31 まで | 50 trials まで | 取得時に end が 2023-12-31 にキャップされ、評価エンジン入口でも同日でクリップされます。**Pine Script エクスポートは完全ブロック** |
| Monthly | 制限なし | 制限なし | 月額サブスクリプション。最新データ・無制限 trial で取得・評価可能。Pine Script エクスポート可 |
| Annual | 制限なし | 制限なし | 年額サブスクリプション。最新データ・無制限 trial で取得・評価可能。Pine Script エクスポート可 |
| Lifetime | 制限なし | 制限なし | 買い切り。最新データ・無制限 trial で取得・評価可能。Pine Script エクスポート可 |

![Free プランと有料プランの機能比較カード](../assets/illustrations/guides/freemium-free-vs-pro-plans.png)
...
![Free プランのデータ日付クリップ図](../assets/illustrations/guides/freemium-data-date-clip.png)
...
- `forge optimize run / cross-symbol / portfolio / multi-portfolio / walk-forward / grid` のいずれも、Free プランでは trial 数が **50 回** にキャップされます。エラー終了せずキャップ後の値で実行を続行します。
...
- Free プランでは **ハードブロック**：いずれのコマンドも実行直後に完全停止し、ファイル出力も標準出力も行われません。
...
- `forge pine import` はインポート機能のため対象外で、Free プランでも継続して利用できます。

Pine Script エクスポートのハードブロック表示例（Free プラン・CLI）：
```text
╭─────────────── 🔒 有料プラン限定機能 ───────────────╮
