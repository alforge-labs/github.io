...
!!! note "対象コマンド"
    ...
    取得時にも評価時にも **2023-12-31** をキャップとして共有し、最適化系コマンドは **50 trials** をキャップとして共有しています。

## プラン構成

| プラン | データ取得・評価の日付制限 | 最適化 trial 数 | 補足 |
|---|---|---|---|
| Free | 2023-12-31 まで | 50 trials まで | 取得時に end が 2023-12-31 にキャップされ、評価エンジン入口でも同日でクリップされます |
| Monthly | 制限なし | 制限なし | 月額サブスクリプション。最新データ・無制限 trial で取得・評価可能 |
| Annual | 制限なし | 制限なし | 年額サブスクリプション。最新データ・無制限 trial で取得・評価可能 |
| Lifetime | 制限なし | 制限なし | 買い切り。最新データ・無制限 trial で取得・評価可能 |
...
制限は一切発動せず、最新データ・無制限 trial で取得・評価できます。出力にも `freemium_limit_notices` の警告は載りません。
