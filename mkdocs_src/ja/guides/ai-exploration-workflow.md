...
```json
{
  "permissions": {
    "allow": [
      "Write(/絶対パス/alpha-strategies/data/strategies/*.json)",
...
`/絶対パス/` は実環境のパス（例: `/Users/yourname/dev/alpha-trade`）に置き換えてください。

| パターン | 許可される操作 |
|---------|--------------|
| `Write(.../strategies/*.json)` | 戦略 JSON の一時ファイル生成（1 戦略ごと） |
