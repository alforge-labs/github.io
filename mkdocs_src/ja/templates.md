...
!!! info "本ページの位置づけ"
    - 配布バイナリ同梱の **7 戦略の ID 一覧** は [CLI リファレンス: forge strategy](cli-reference/strategy.md) を参照してください。
    - 本ページの JSON は「自分でカスタム戦略を作るときの組み立て例」として書かれており、`strategy_id` は `my_*` の仮名にしています。実際に使う場合は意味のある自分の命名に置き換えて、`forge strategy save` で登録してください。
...
- **最適化対象**: `forge optimize run QQQ --strategy my_hmm_bb_rsi_v1 --metric sharpe_ratio --save`
...
- **クロスシンボル最適化**: `forge optimize cross-symbol GC=F SI=F CL=F --strategy my_regime_switching_v1 --aggregation min --save`
